...
// Sender dispatches transactional SMS. No provider is integrated yet: until one
...
//
// Adding a real provider (MSG91, Twilio, …) means implementing this interface
...
func New(apiKey, from string) Sender {
	...
	log.Printf("[sms] SMS_API_KEY is set but no provider is implemented — logging instead")
	return &logSender{from: from}
}
...
func (l *logSender) Send(_ context.Context, to, body string) error {
	log.Printf("[sms/log] (no provider configured) to=%s from=%s body=%q", to, l.from, body)
	return nil
}
